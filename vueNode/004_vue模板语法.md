...
这个 `span` 的内容将会被替换成为属性值 `rawHtml`，直接作为 HTML——**会忽略解析属性值中的数据绑定**（比如span中的文本是{{ msg }}，不会渲染到页面上）。注意，你不能使用 `v-html` 来复合局部模板，因为 Vue 不是基于字符串的模板引擎。反之，对于用户界面 (UI)，组件更适合作为可重用和可组合的基本单位。
